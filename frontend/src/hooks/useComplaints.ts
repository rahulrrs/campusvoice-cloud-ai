      ...
      const pending = (await getPendingComplaints())
        ...
        .map((p) => {
          return {
            ...
            analysis: p.data.analysis as ComplaintAnalysisBundle | undefined,
...
export async function syncOfflineComplaints(userId: string) {
  ...
  for (const p of mine) {
    try {
      ...
      payload.analysis = p.data.analysis as ComplaintAnalysisBundle | undefined;
      payload.source_language = p.data.source_language;
