...
# --- mappings created by clean_dataset.py (in OUT_DIR) ---
...
print(f"After capping (max {MAX_PER_CLASS}/class) total training samples: {len(train_df)}")
...
for i in range(num_labels):
    ...
    label_w[i] = 1.0 if c == 0 else (len(train_df) / (num_labels * c))
...
class DistilBertMultiTask(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, priority_labels=None, **kwargs):
        out = self.backbone(input_ids=input_ids, attention_mask=attention_mask)
        pooled = self.dropout(out.last_hidden_state[:, 0])

        label_logits = self.label_head(self.label_dropout(pooled))
        prio_logits = self.prio_head(self.prio_dropout(pooled))
...
print("Training (single-label + priority)...")
trainer.train()
