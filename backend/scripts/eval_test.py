...
for path in (label_map_path, prio_map_path):
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}")

with open(label_map_path, "r", encoding="utf-8") as file:
    id_to_label = {int(k): v for k, v in json.load(file).items()}
with open(prio_map_path, "r", encoding="utf-8") as file:
    id_to_priority = {int(k): v for k, v in json.load(file).items()}
